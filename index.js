...
r.setRoutes({
   ...
   "/docs/gettingStarted": {
      getComponent: lazy("/pages/getStarted.js"),
      layout(s) {
         return docLayout({ pos: 1 }, s);
      },
   },
   "/docs/creatingElements": {
      getComponent: lazy("/pages/creatingElements.js"),
      layout(s) {
         return docLayout({ pos: 2 }, s);
      },
   },
   "/docs/creatingComponents": {
      getComponent: lazy("/pages/creatingComponents.js"),
      layout(s) {
         return docLayout({ pos: 3 }, s);
      },
   },
   "/docs/reactivityStates": {
      getComponent: lazy("/pages/reactivityStates.js"),
      layout(s) {
         return docLayout({ pos: 4 }, s);
      },
   },
   "/docs/componentTutorial": {
      getComponent: lazy("/pages/componentTutorial.js"),
      layout(s) {
         return docLayout({ pos: 5 }, s);
      },
   },
   "/docs/renderingLists": {
      getComponent: lazy("/pages/renderingLists.js"),
      layout(s) {
...
function docSideNav(pos) {
   return aside(
      ul(
         ...
         li(
            r.Link({ href: "/docs/creatingComponents" }, "creating components")
         ),
         li(
            r.Link({ href: "/docs/reactivityStates" }, "reactivity with states")
         ),
         li(
            r.Link(
               { href: "/docs/componentTutorial" },
   ...
   return div(
      ...
      main(
         ...
         div(
            ...
            div(
               a(
                     { href: "https://dominitydocs.vercel.app/docs/intro" },
                     button("get started", { class: "btn" })
                  )
                  .on("click", (e) => {}),
                  r.Link(
                     { href: "/docs/componentTutorial" },
                     button("see components", { class: "btn outlined" })
                  )
            ).css({
               display: "flex",
            })
         ...
         div(
            ...
            ul(
               ...
               li(
                  b(".showIf()", { class: "tag" }),
                  span(
                     "- conditional rendering depending on truthiness of a state"
                  )
               ),
               li(
...
function Footer() {
   ...
   return footer(
      ...
      div(
         ...
         span(`dominity docs arent done cause I have college and i dont have time to write docs all day and domiity doesnt have a docs tool like docosaurus cause thats clearly out of scope dominity isnt tryna be like react or anything
               the documentation will be written in docusaurus rn and will not feature dominity , but if u have time and sympathy to help out please reach out 🙏
            `).css({
               padding:'1rem',
               textAlign:'center'
            })
